...
(function () {
    ...
    const Sidenotes = {
        ...
        config: {
            ...
            sidenoteWidth: 320,           // 增加宽度以适应图片
            marginTop: 20,
            spacing: 30,
            maxSidenoteHeight: 150,       // 调整最大高度
            collapseThreshold: 120        // 调整触发阈值
        },
        ...
        convertFootnotesToSidenotes() {
            ...
            this.footnotes.forEach((footnote, index) => {
                ...
                content.className = 'sidenote-content';
                content.innerHTML = footnote.content;

                // 组装结构
                innerWrapper.innerHTML = `<span class="sidenote-number">${footnote.number}</span>`;
                ...
                moreIndicator.innerHTML = '<span>···</span>';
                ...
                footnote.link.addEventListener('click', (e) => {
                    ...
                    if (window.innerWidth >= this.config.minWidthForSidenotes) {
                        // 如果是折叠的侧注，先展开
                        if (sidenote.classList.contains('cut-off') && !sidenote.classList.contains('expanded')) {
                            this.expandSidenote(sidenote);
                        }
        ...
        setupLongSidenotes() {
            this.sidenotes.forEach((sidenote) => {
                ...
                if (contentHeight > this.config.collapseThreshold) {
                    ...
                    outerWrapper.style.overflowX = 'hidden'; // 防止横向滚动
        ...
        addExpandToggle(sidenote) {
            ...
            moreIndicator.style.userSelect = 'none';
            ...
            const toggleExpand = (e) => {
                ...
                if (element.classList.contains('expanded')) {
                    // 折叠
                    this.collapseSidenote(element);
                } else {
                    // 展开
                    this.expandSidenote(element);
                }
            };
            
            // 绑定点击事件到整个指示器区域
            moreIndicator.addEventListener('click', toggleExpand);
            
            // 为触摸设备添加支持
            moreIndicator.addEventListener('touchend', (e) => {
                e.preventDefault();
                toggleExpand(e);
            });
            
            // 存储toggle函数供其他地方使用
            sidenote.toggleExpand = toggleExpand;
        },

                // 展开侧注
        expandSidenote(element) {
            const sidenote = this.sidenotes.find(s => s.element === element);
            if (!sidenote) return;
            
            element.classList.add('expanded');
            sidenote.outerWrapper.style.maxHeight = 'none';
            sidenote.moreIndicator.innerHTML = '<span>×</span>';
            element.classList.remove('show-more-indicator');
        },
        
        // 折叠侧注
        collapseSidenote(element) {
            const sidenote = this.sidenotes.find(s => s.element === element);
            if (!sidenote) return;
            
            element.classList.remove('expanded');
            sidenote.outerWrapper.style.maxHeight = `${this.config.maxSidenoteHeight}px`;
            sidenote.moreIndicator.innerHTML = '<span>···</span>';
            element.classList.add('show-more-indicator');
            
            // 滚动到顶部
            sidenote.outerWrapper.scrollTop = 0;
        },
